...
The V3.4 sampler definition supplies the Native Masked method to workflows that do not yet serialize the new final widget. Maintained V3.4 templates should use `Continuation Method = Native Masked — exact continuation (Recommended)` and `Continuity = Strong — 39 frames` when generated Audio Continuity is enabled. Guide / Motion Context reproduces the previous guide/RoPE continuation architecture.
